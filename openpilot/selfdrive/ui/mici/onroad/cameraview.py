...
class CameraView(Widget):
  def __init__(self, name: str, stream_type: VisionStreamType, cover: bool = False):
    super().__init__()
    self._name = name
    # cover=True crops the source to the widget aspect and fills it (no letterbox bars).
    # Default False keeps the existing letterboxed behaviour for the onroad views.
    self._cover = cover
    # Primary stream
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    if self._cover:
      # Crop the source to the widget's aspect ratio and draw it over the whole rect, so the
      # frame fills the screen with no black bars (centre-crops the excess instead of scaling).
      widget_aspect = rect.width / rect.height
      frame_aspect = self.frame.width / self.frame.height
      if frame_aspect > widget_aspect:  # frame too wide -> trim left/right
        sw = self.frame.height * widget_aspect
        src_rect = rl.Rectangle((self.frame.width - sw) / 2, 0, sw, float(self.frame.height))
      else:                             # frame too tall -> trim top/bottom
        sh = self.frame.width / widget_aspect
        src_rect = rl.Rectangle(0, (self.frame.height - sh) / 2, float(self.frame.width), sh)
