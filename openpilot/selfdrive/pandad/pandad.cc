...
std::optional<bool> send_panda_states(PubMaster *pm, Panda *panda, bool is_onroad, bool spoofing_started) {
  ...
  //
  // Stock gates both of these on ignition, and on this hardware that is circular:
  // ignitionLine is a harness sense pin an OBD-C cable doesn't have, so ignition can only
  // come from ignitionCan -- but that needs CAN, CAN needs the OBD multiplexer, and stock
  // only configures the multiplexer once already onroad, which needs ignition. Net effect
  // with stock gating: power save never released, mux never set, `can` permanently empty.
